    ...
    public function addAction() {
        ...
        if ($pk_value) {//编辑
            ...
            if ($pk_value == ADMIN_ROLE_ID && $this->_admin_info[$pk_field] != ADMIN_ROLE_ID) {//不可编辑指定角色。增加当前角色id判断 by mrmsl on 2012-07-05 08:50:27
                $this->_model->addLog(L('TRY,EDIT,MODULE_NAME_ROLE') . "{$pk_field}: {$pk_value}", LOG_TYPE_INVALID_PARAM);
                $this->_ajaxReturn(false, L('EDIT,FAILURE'));
            }

            if (!isset($cache_data[$pk_value])) {//角色不存在
                $this->_model->addLog(L("EDIT,MODULE_NAME_ROLE,FAILURE,%<br />,INVALID_PARAM,%:,MODULE_NAME_ROLE,%{$pk_field}({$pk_value}),NOT_EXIST"), LOG_TYPE_INVALID_PARAM);
                $this->_ajaxReturn(false, L('EDIT,FAILURE'));
            }
            ...
            if ($this->_model->save() === false) {//更新出错
                $this->_sqlErrorExit(L('EDIT,MODULE_NAME_ROLE') . "{$role_info[$this->_name_column]}({$pk_value})" . L('FAILURE'), L('EDIT,FAILURE'));
            }
            ...
            //管理员操作日志
            $this->_model->addLog(L('EDIT,MODULE_NAME_ROLE')  . "{$role_info[$this->_name_column]}({$pk_value})." . $diff. L('SUCCESS'), LOG_TYPE_ADMIN_OPERATE);
            ...
            if (($insert_id = $this->_model->add()) === false) {//插入出错
                $this->_sqlErrorExit(L('ADD,MODULE_NAME_ROLE') . $insert_data . L('FAILURE'), L('ADD,FAILURE'));
            }

            //权限
            $priv_id && $this->_model->setRolePriv($insert_id, $priv_id);

            $this->_model->addLog(L('ADD,MODULE_NAME_ROLE') . $insert_data . L('SUCCESS'), LOG_TYPE_ADMIN_OPERATE);
