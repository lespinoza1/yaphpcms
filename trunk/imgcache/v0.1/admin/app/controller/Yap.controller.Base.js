 ...
 *
 * @file            app/controller/Yap.controller.Base.js
 * @version         0.1
 * @abstract
...
Ext.define('Yap.controller.Base', {
    ...
    addAction: function(data, options) {
        ...
        pkValue && Yap.cmp.viewport.setPageTitle(data.controller, data.action, document.title.replace(data.text, title));
        ...
        Yap.cmp.card.layout.setActiveItem(this._formpanel);
    },//end addAction
    ...
    edit: function(record, setEditRecord, extraParam) {
        ...
        Yap.History.push('{action}&{idProperty}={pkValue}&back={back}{extraParam}'.format({
    ...
    formPanel: function(data, options) {
        ...
        if (!this._formpanel || destroy) {
            ...
            this._formpanel = Ext.create('Yap.ux.Form', cfg);
        }
     ...
     * @param {Object} [options] Yap.ux.Grid/Yap.ux.TreeGrid配置项
     *
     * @return {void} 无返回值
     */
    listAction: function(data, options) {
        ...
        if (data.sort) {//可排序，通常为普通列表
            ...
            Yap.cmp.card.layout.setActiveItem(this.treegrid(data, options));
     ...
     * @param {Object} [options] Yap.ux.Grid/Yap.ux.TreeGrid配置项
     *
     * @return {Object} Yap.ux.Grid/Yap.ux.TreeGrid实例
     */
    listgrid: function(data, options) {
        ...
        if (!this._listgrid) {
            ...
            this._listgrid = Ext.create(cfg.appgridtype || 'Yap.ux.Grid', cfg);
        }
    ...
    setHistory: function(data) {
        var tab = Yap.cmp.tabs.getActiveTab();
        Ext.apply(tab, data);
        var href = Ext.Object.toQueryString(tab);
        Yap.History.push(href, true);
    ...
    tabs: function(activeTab) {
        Ext.require('Yap.ux.TabPanel', function() {
            var me = this;

            if (!this._tabs) {
                this._tabs = Ext.create('Yap.ux.TabPanel', {
                    controller: this
                });
            };

            Yap.cmp.card.layout.setActiveItem(this._tabs);
     ...
     *
     * @param {Object} data 当前标签数据
     * @param {Object} options Yap.ux.TreeGrid配置项
     *
     * @return {Object} Yap.ux.TreeGrid实例
     */
