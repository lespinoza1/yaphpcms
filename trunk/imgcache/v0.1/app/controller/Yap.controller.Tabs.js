 ...
 * 标签选项卡控制器
 *
 * @file            app/controller/Yap.controller.Tabs.js
 * @version         0.1
...
Ext.define('Yap.controller.Tabs', {
    ...
    addTabListeners: function(view) {
        view.el.on({
            ...
            contextmenu: function(e, element) {//右键菜单 by mrmsl on 2012-07-28 16:39:36
                ...
                var tabs = Yap.cmp.tabs;//标签容器
                tabs.activeTab != controller && Yap.History.push(el.getAttribute('href'));//活跃标签
                var recentTabs = tabs.recentTabs;//最近操作
                var recentItems = [];//最近操作 Ext.menu.items

                Ext.each(recentTabs, function(item, index) {//最近操作
                    recentItems.push({
                        text: (index + 1) + '. ' + Yap.cmp.viewport.cache[item.controller + item.action],
                        handler: function() {
                            Yap.History.push(item);
                        }
                    })
                });
    ...
    execScript: function(Class, data) {
        ...
        if (!method) {//方法不存在
            this.actionNotExist('Yap.controller.' + Ext.String.capitalize(controller), action + 'Action');
            return false;
        }

        var store = Yap.cmp.tree.selectUrl(controller, action);//高亮选中菜单

        data.text = store ? store.get('menu_name') : lang('NO_TITLE');

        Yap.cmp.viewport.setPageTitle(controller, action);//设置页面标题
    ...
    loadScript: function(hash, nav) {
        ...
        nav || Yap.History.push(hash, true);
        ...
        if (this.cache[controller]) {//已经加载
            ...
            //本地环境，加载未压缩js
            var url = System.IS_LOCAL ? Ext.Loader.getPath('Yap#controller') + controllerClassName + '.js' : Ext.Loader.getPath('Yap#pack') + controllerClassName + '.pack.js';
            ...
            var request = Ext.data.JsonP.request({
                ...
                success: function(require, className) {
                    ...
                    else {//[['Yap.store.Admin', 'Yap.store.store.Role'], Ext.define('Yap.controller.Admin') {}]
                        className = require[1];
                        require = require[0];
     ...
     * @param {Mixed}    [require]  依赖js数组，务必Ext.Loader.setPath()已设有脚本前缀，如'Yap.store.Admin': System.sys_base_js_url + 'app/store/Yap.store.Admin.js'
     *
     * @return {void} 无返回值
     */
