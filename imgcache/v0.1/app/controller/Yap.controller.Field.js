 ...
 * 表单域控制器
 *
 * @file            app/controller/Yap.controller.Field.js
 * @version         0.1
...
Ext.define('Yap.controller.Field', {
    ...
    formField: function(data) {
        ...
        ]]),
        {
            ...
            store: Ext.create('Yap.store.Tree', {
                folderSort: false,
...
#{%PLEASE_ENTER,@field_name}#MUST_VALIDATE#notblank', {
    ...
    /**
     * @inheritdoc Yap.controller.Admin#pagingBar
     */
     ...
     */
    store: function(data) {
        this._store = this._store || Ext.create('Yap.store.Field');
    ...
    tbar: function(data) {
        ...
        return {
            ...
            }, '-', Yap.Field.combo().enable(),//显示状态 by mrmsl on 2012-09-15 14:23:08
            /*if (Ext.isString(this.singleSelectValueField)) {//表单域名
             ...
             Yap.Field.field().textField('menu_id', '', '', data.menu_id, {itemId: 'menu_id', hidden: true}),
            {
                xtype: 'treepicker',
                name: '_menu_id',
                value: data.menu_id,
    ...
    //放到最后定义，否则，jsduck后，上面的方法将属于Yap.store.Field或Yap.model.Field
    /**
     * 定义模型
     *
     * @author        mrmsl <msl-138@163.com>
...
//放到最后，以符合生成jsduck类说明
Ext.data.JsonP.Yap_controller_Field(Yap.controller.Field);