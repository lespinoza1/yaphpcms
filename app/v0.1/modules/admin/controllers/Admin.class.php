...
class AdminController extends BaseController {
    ...
    public function addAction() {
        ...
        $log_msg   = $msg . L('MODULE_NAME_ADMIN,FAILURE');//错误日志
        ...
        if ($pk_value) {//编辑
            ...
            if ($pk_value == ADMIN_ID && $this->_admin_info[$pk_field] != ADMIN_ID) {//不可编辑指定管理员。增加当前管理员id判断 by mrmsl on 2012-07-05 08:52:14
                $this->_model->addLog(L('TRY,EDIT,MODULE_NAME_ADMIN') . "{$pk_field}: {$pk_value}", LOG_TYPE_INVALID_PARAM);
            ...
            if ($this->_model->save() === false) {//更新出错
                $this->_sqlErrorExit($msg . L('MODULE_NAME_ADMIN') . "{$admin_info['username']}({$pk_value})" . L('FAILURE'), $error_msg);
            }
            ...
            $this->_model->addLog($msg . L('MODULE_NAME_ADMIN')  . "{$admin_info['username']}({$pk_value})." . $diff. L('SUCCESS'), LOG_TYPE_ADMIN_OPERATE);
            ...
            if ($this->_model->add() === false) {//插入出错
                $this->_sqlErrorExit($msg . L('MODULE_NAME_ADMIN') . $data . L('FAILURE'), $error_msg);
            }

            $this->_model->addLog($msg . L('MODULE_NAME_ADMIN') . $data . L('SUCCESS'), LOG_TYPE_ADMIN_OPERATE);
            $this->_setCache()->_ajaxReturn(true, $msg . L('SUCCESS'));
        }
    }//end addAction
    ...
    public function changePasswordAction() {
        ...
        $_auto_validation = array_merge($_auto_validation, $this->_model->getValinew_date('password,_password_confirm', false));
    ...
    public function listAction() {
        ...
        $data === false && $this->_sqlErrorExit(L('QUERY,MODULE_NAME_ADMIN') . L('LIST,ERROR'));//出错
